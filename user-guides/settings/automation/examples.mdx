...
Adapt these patterns to the fields, statuses, recipients and provider options available in your account. Start with Specific Companies, inspect Run Test results, and save inactive until the real actions are ready to run.
...
<Steps>
  ...
  <Step title="Trigger">
    ...
    Exactly 30 days defines the matching date window. Actual execution still depends on the rule being active, evaluation and duplicate suppression.
  </Step>
  ...
  <Step title="Action 2: Add to Planner">
    ...
    - **Due Offset (days)**: 30
      ...
      The task is created with "Not Started" status by default. Use the available Description and Planner Type fields; edit the resulting task for other details.
  </Step>
</Steps>
...
Check hourly for tickets currently in **Resolved** status and send the configured request. These conditions do not establish how long a ticket has been resolved, and the Primary Contact recipient is not necessarily the person who opened the ticket.

<Steps>
  <Step title="Basic Information">
    - **Name**: CSAT Survey on Resolved Tickets
    - **Scope**: All Companies
    - **Description**: Request feedback for matching resolved tickets on the hourly evaluation.
  </Step>
  <Step title="Trigger">
    - **Entity**: Ticket
    - **Trigger**: Scheduled
  ...
  <Step title="Conditions">
    ...
    The `survey_sent` custom checkbox field acts as the "have we already done this?" gate. Set it up once under **Settings > Custom Fields** for the Ticket entity. Check how existing records initialize this flag, and keep Once per entity dedup unless repeat delivery is intentional. A flag update is not a delivery receipt.
  </Step>
  <Step title="Action 1: Send Email">
...
<Note>
Scheduled rules offer **Once per entity** dedup or **Every run**. Choose deliberately. A custom done flag can encode a business workflow, but action failure or pending approval can separate flag state from actual delivery. Inspect execution results before relying on it.
</Note>

### 5. Stale Goal Nudge

Once a week at the configured time, alert account managers about in-progress goals below 50% whose target date is more than 60 days overdue. These conditions measure an overdue target, not the time since the last progress update.
...
- **Action**: In-App Alert to Company Teams &rarr; Account Manager role
  ...
  - **Message**: `{{entity.name}} at {{entity.company_name}} has a target date over 60 days overdue and is below 50% progress.`
...
- **Conditions**:
  ...
  - <code style={{ overflowWrap: "anywhere" }}>custom_fields.remediation_task_created</code> is **False**
- **Actions**:
...
Every evening, find meetings in Pending Notes that have agenda items and alert the selected users. These conditions do not restrict matches to meetings that ended today or dynamically select each organizer.
...
- **Action**: In-App Alert
  - **Alert Type**: Info
  - **Title**: `Add meeting notes: {{entity.title}}`
...
3. **Choose dedup and recipients deliberately.** Review Once per entity versus Every run. Use a done flag only when its state accurately represents your workflow.
...
<AccordionGroup>
  <Accordion title="Does an hourly schedule mean one hour after a status changed?">
    No. It controls evaluation frequency. A Resolved condition selects the current status; it does not establish the duration in that status.
  </Accordion>
  <Accordion title="Does Primary Contact mean the ticket requester?">
    No. It selects the company’s configured PSA contact type. Verify the recipient choice or use the supported survey workflow for the intended audience.
  </Accordion>
  <Accordion title="Can I paste the sample survey-link placeholder and enable the rule?">
    No. Replace placeholders with a valid intended workflow or configure Send Survey. Previewing matches does not verify that an email contains a usable survey invitation.
  </Accordion>
  <Accordion title="Does the stale-goal example measure the last update?">
    No. Its conditions measure target-date age, status and progress. Use wording that describes those fields accurately.
  </Accordion>
  <Accordion title="Will the meeting example notify each meeting’s owner automatically?">
    Specific Users targets the users you select. It does not resolve a templated organizer ID for each meeting; also add a date condition if you need only today’s meetings.
  </Accordion>
  <Accordion title="How do I safely adapt a preset or copied rule?">
    Keep it inactive, narrow company scope, select valid statuses and recipients, review templates and test matches. Enable only when the real writes or messages are intended.
  </Accordion>
</AccordionGroup>
...
<CardGroup cols={2}>
  ...
  <Card title="Automation Overview" icon="robot" href="/user-guides/settings/automation/index">
