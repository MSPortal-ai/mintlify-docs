...
<AccordionGroup>
  ...
  <Accordion title="Send Email" icon="envelope">
    ...
    **Recipients** are picked using the Recipient Selector (shown as the **Send To** dropdown):
    ...
    {/* Post-release follow-up: verify Fallback recipient in production, capture the Send Email recipient controls, and remove the branch-preview notice. */}

    <Note>
    Branch preview for the September 18 release. The fallback recipient controls are not yet published.
    </Note>
