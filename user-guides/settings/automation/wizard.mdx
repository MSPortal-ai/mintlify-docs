...
<Steps>
  ...
  <Step title="Review and save">
    Use **Run Test** in Setup to inspect matches, then click **Next** to review the rule and its Active setting before choosing **Create Rule**.
  </Step>
</Steps>
...
Choose a trigger supported for the selected entity. Date Threshold and Scheduled evaluate stored records. Where offered, Survey Response Received and Onboarding Event respond to their matching events; changing the entity can change the available triggers.
...
<Tabs>
  <Tab title="Date Threshold">
    ...
    **Configuration fields:**
    ...
    Date Threshold rules evaluate in the background. The configured date window determines eligibility; execution logs establish when actions actually ran.
  </Tab>
  <Tab title="Scheduled">
...
Conditions filter which records the rule acts on. The Conditions section sits at the top right of the Setup step. **A rule with no conditions adds no further filter to records selected by its trigger** within the rule's scope.
...
<Note>
Dynamic dropdowns pull their options from your live data. For PSA options, complete the relevant reference-data sync and reopen the selector after changing provider configuration. A value absent from synchronized data may not yet appear.
</Note>
...
The editor offers the following action types where supported for your entity. Each one has its own configuration form that opens in an accordion when you add it.
...
<AccordionGroup>
  <Accordion title="Add to Planner" icon="list-check">
    ...
    | **Due Offset (days)** | For individual items, an offset from execution. Bundled dates can use the record date and trigger/action offsets; inspect the bundle preview |

    The created task defaults to "Not Started". The action also offers Description and Planner Type, with date controls where applicable. Edit the created Planner item for other details. An Update Field action targets the original matched entity, not the newly created Planner item.
  </Accordion>

  <Accordion title="Set Relative Date">
    Choose an available source date, a writable target date and a positive or negative offset in days, weeks, months or years. The editor also offers supported current-date or trigger-date sources. This writes the matched record when the rule executes; preview the date logic before enabling the rule.
  </Accordion>
  ...
  <Accordion title="Send Email" icon="envelope">
    ...
    **Recipients** are picked using the Recipient Selector (shown as the **Send To** dropdown):
    ...
    | **Role Members** | Team members assigned to the affected company in selected roles (for example, Account Manager, vCIO) |
    ...
    When **Send To** is set to **Role Members**, a **Fallback recipient** field appears below the roles. Choose **MSPortal team member** and select a person, or choose **Email address** and enter one address. The fallback is optional in the editor.

    The fallback receives the email only when no active member of the selected roles has a valid email for the affected company, or the record has no company. When role members resolve, the fallback is not emailed. The team member list shows active MSP staff with an email address; company users do not appear. MSPortal checks that the selected member is still active when you save the rule and again when the email sends.
  ...
  <Accordion title="Send Survey" icon="clipboard-check">
    ...
    | **Invite note** (optional) | Short message added to the invite email. Supports entity variables such as `{{entity.title}}`. Max 2000 characters |

    For Ticket rules, the invite names the ticket. The default survey email adds a line such as "This survey is about ticket T20260918.0022: Printer offline" under the greeting, in the recipient's language. Custom survey email templates can place the same details with `{{ticket_reference}}`, `{{ticket_number}}` and `{{ticket_title}}`, listed under **Available Variables** when you edit a template in **Surveys > Settings > Email Templates**. Surveys sent without a ticket leave these blank.
  </Accordion>
...
### Team membership actions (branch preview)

**Add team members** and **Remove team members** change the team of the company associated with the triggering record. Choose a role and staff; removal also supports everyone in that role. Global roles can propagate additions to other companies, while removals create company-specific exceptions. Records without a company are skipped.

See [Global Team Roles](/user-guides/settings/global-team-roles) for prerequisites, scope effects and the onboarding handoff example. These actions do not support an approval hold.
...
Click **Next** in the bottom-right to move to the Review step. Review the summary and Active setting before saving.
...
Return to Setup for the **Test Rule** section. Click **Run Test** to dry-run the rule against live data. The test returns a match count and a sample table, so you can confirm:
...
When you are satisfied with the test results, click **Create Rule** in the bottom-right. The rule is saved and appears on the list. New rules start inactive. Check the **Active** setting on Review before saving. An active rule can execute on its next eligible cycle; save it inactive while preparing a controlled rollout.

Use **Run Now** only when you intend a real execution of the enabled rule. It queues work; inspect Logs for the outcome and any pending approval.


## Questions and troubleshooting

<AccordionGroup>
  <Accordion title="Will Run Test send my email or open a PSA ticket?">
    No. It evaluates matches without executing actions. It does not prove that recipients, provider credentials or delivery will succeed during a real run.
  </Accordion>
  <Accordion title="Why are fewer rows shown than the match count?">
    The preview can show a sample. Review its count and sampling information; the displayed rows are not necessarily the entire execution set.
  </Accordion>
  <Accordion title="Does Update Field edit the Planner item just created above it?">
    No. It writes the matched entity. Configure supported fields in Add to Planner or edit the resulting Planner item separately.
  </Accordion>
  <Accordion title="What happens if I save an active rule?">
    The saved rule becomes eligible for automatic execution. Inspect Scope, recipients, action order and approval settings before saving it active.
  </Accordion>
  <Accordion title="What prevents a Scheduled rule from repeating work?">
    Its Dedup choice matters. Once per entity suppresses repeat processing for that rule; Every run intentionally allows recurring actions. A custom done flag is an additional workflow choice, not proof of exactly-once delivery.
  </Accordion>
  <Accordion title="Why is a field or action unavailable?">
    Available options depend on the entity, supported fields, permissions and provider data. Choose a supported option rather than entering an arbitrary field key to bypass the editor.
  </Accordion>
</AccordionGroup>
