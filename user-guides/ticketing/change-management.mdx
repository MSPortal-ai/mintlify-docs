...
<Steps>
  ...
  <Step title="Choose the approver company role">
    Select the **Approver company role**. When a change is requested on a ticket, eligible users assigned to that company role at that client are resolved as recipients when the request is created.
  </Step>
...
<Warning>
Assign eligible approvers before requesting a change. Without them, the request cannot follow the intended approval workflow. Assign users to the role first, under **Settings > Users & Roles**.
</Warning>
...
<Info>
Only users recorded as approvers for that pending request can approve or reject. Everyone else sees the request but has no decision buttons, even administrators.
</Info>
...
<AccordionGroup>
  <Accordion title="Does saving approval settings approve existing requests?">
    No. It configures future request handling. Open each pending request to inspect its recorded approvers and decision state.
  </Accordion>
  <Accordion title="Can an administrator approve without being a recipient?">
    Broad administration access is not an approval vote. The decision action requires the user to be a recorded approver for that pending request.
  </Accordion>
  <Accordion title="Does approval mean the change has been completed?">
    No. With a window it becomes Scheduled; without one it remains Approved. Start change and Complete change are separate lifecycle actions for supported portal-managed requests.
  </Accordion>
  <Accordion title="Do freeze windows block emergency scheduling?">
    They raise conflict warnings rather than preventing scheduling. Review the overlap and your operating process before requesting or running the change.
  </Accordion>
  <Accordion title="Does No scheduling conflicts detected guarantee the window is safe?">
    No. It reflects the records and scope available to the check. Review the client’s operations and data freshness; it is not proof that every external calendar or dependency was checked.
  </Accordion>
  <Accordion title="Can I run a provider-owned change from the portal?">
    Provider-owned changes are read-only in this workflow. Open the source PSA to manage them; portal-owned lifecycle controls do not apply to every imported change.
  </Accordion>
  <Accordion title="Does changing the approver role add voters to an existing request?">
    Approvers are recorded when the request is created. Inspect the pending request rather than assuming later company-role edits rewrite its recipients.
  </Accordion>
  <Accordion title="What should I do if an Autotask-owned decision fails?">
    Read the error and verify the source approval state before retrying. The provider update and portal result are both relevant; do not treat a clicked Approve button as confirmed success.
  </Accordion>
</AccordionGroup>
