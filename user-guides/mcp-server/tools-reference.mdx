...
| Action / legacy name | Public tool | Access | Permissions | Description |
| --- | --- | --- | --- | --- |
| `get_security_summary` | `query_security` | Read | `read_security` | Cross-provider security posture summary: Cork (compliance findings, vulnerabilities incl. critical/known-exploited, endpoints), Acronis EDR (incident counts by state/severity/mitigation, endpoint protection), and Huntress (agents, escalations, incident reports, remediations, SAT learner counts), and Avanan email security over the last 30 days (threats by phishing/malware/other, open threats, spam and graymail blocked, protected users). A provider section is null when its data is unavailable; zero counts mean no data synced. Pair with get_secure_score_summary (M365) for the full QBR risk story. |
| `list_avanan_email_threats` | `query_security` | Read | `read_security` | List Avanan (Check Point Harmony Email & Collaboration) email threats from the last 30 days, newest first: threat type (phishing, malware, suspicious, DLP, malicious URL, and so on), state (new/detected/pending are open; remediated and dismissed are closed), severity (critical/high/medium/low), sender, the protected email tenant and mapped company, remediation actions taken, and a link to the Avanan portal. Search matches sender, description, tenant and company name. |
...
## Personal task lists — branch preview

The private-task-list branch adds the following staff-only operations. Availability depends on the deployed catalog; use the schemas returned by your connection. Task tools use the existing `mcp.read` and `mcp.write` grants.

| Operation | Purpose | Access |
| --- | --- | --- |
| `list_my_tasks` | Read your own tasks or your assigned companies' tasks | Read |
| `create_tasks` | Propose up to 100 new tasks, optionally with a company and source link | Write; confirmation required |
| `update_task` | Propose an edit to an editable task | Write; confirmation required |
| `complete_tasks` | Complete a selected batch atomically | Write; confirmation required |
| `delete_tasks` | Propose deletion of a selected batch | Write; confirmation required |

Ownership, company sharing and tenant boundaries are enforced when a change is committed. Private tasks can also be read by authorized task-list auditors in the portal. Scratch notes are not exposed by these tools. See [Tasks and notes](/user-guides/tasks/index) for the portal workflow.
