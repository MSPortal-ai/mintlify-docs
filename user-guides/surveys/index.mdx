...
Open the page’s **AI Survey Designer** using the assistant control available in your account.
...
The recipient count updates as you make selections. Inspect the resolved recipients before sending; a group count is not proof of deliverable email addresses.
...
<CardGroup cols={3}>
  <Card title="Send Now" icon="paper-plane">
    Queue the intended survey invitations after confirmation
  </Card>
...
<Steps>
  ...
  <Step title="Apply Rules">
    ...
    Review the group’s Auto-Update setting and its latest membership result. Saving a rule is not proof that its membership has already refreshed.
  </Step>
</Steps>
...
{/* Post-release follow-up: verify Response notifications in production, capture the survey header button, the Import automation dialog and the Fallback recipient controls, and remove the branch-preview notice. */}
...
Set up an email to your account team each time a questionnaire survey receives a response. The setup copies a built-in automation rule, so you review and enable it in **Settings > Automation**.

The **Response notifications** button appears in a survey's page header when your role has write or manage access to **Automation Rules** settings. Importing the rule requires write access.

<Steps>
  <Step title="Open the setup">
    Open a survey's results page and select **Response notifications**. The **Import automation** dialog opens with the **Survey response email** rule set chosen and the current survey selected.
  </Step>
  <Step title="Choose which surveys notify">
    Keep the selected **Survey**, or pick another active survey. Check **All surveys** to send the notification for every current and future survey instead.
  </Step>
  <Step title="Choose the team role">
    In **Team role**, pick the company role to email, such as vCIO. MSPortal emails the members assigned to that role for the company of each response.
  </Step>
  <Step title="Set a fallback recipient">
    In **Fallback recipient**, choose **MSPortal team member** and select a person (only active staff with an email address are listed), or choose **Email address** and enter one address. A fallback is required here.
  </Step>
  <Step title="Import and review">
    Select **Import rule set**. MSPortal creates the rule disabled and opens it in the rule editor. Review the recipients, subject and body, then enable the rule when you are ready.
  </Step>
</Steps>

The imported rule is named **Survey response email notification**, followed by the survey title and the role name, separated by hyphens. Rules for **All surveys** leave out the survey title. If a rule with that name already exists, the import stops. Rename or remove the existing rule before importing again.

The fallback recipient receives the email only when no active member of the selected role has a valid email for the response's company, or the response has no company. See [Send Email](/user-guides/settings/automation/wizard#action-types) for how fallback recipients work in any rule.

The same rule set is available from **Settings > Automation** with the **Import** button.
...
<Steps>
  ...
  <Step title="Save">
    Click **Save**. Subsequent qualifying responses request alerts to the saved recipients. Inspect delivery results if an expected notification does not arrive.
  </Step>
</Steps>
...
<AccordionGroup>
  <Accordion title="Does accepting AI survey changes send invitations?">
    No. It changes the wizard’s draft. Review questions, recipients and delivery settings, then use the appropriate save or send action.
  </Accordion>
  <Accordion title="Does Save Draft start a scheduled campaign?">
    Save Draft preserves the draft without sending. Use the schedule action when you intend to activate scheduled or recurring delivery and verify the saved schedule.
  </Accordion>
  <Accordion title="Why did Schedule Survey not send emails immediately?">
    Scheduled and recurring choices save an active schedule for background processing. Immediate Send Survey is a different choice; inspect the scheduled time and subsequent send history.
  </Accordion>
  <Accordion title="Can I edit a survey without creating a new version?">
    The wizard has a save/edit path and a separate new-version choice where available. Read the footer action before saving; changing an existing version can affect the form recipients use.
  </Accordion>
  <Accordion title="Does a group membership rule only add people?">
    Applying membership rules can remove current members that no longer match as well as add eligible contacts. Review the group and resolved recipients before sending.
  </Accordion>
  <Accordion title="Does Sent mean the recipient submitted a response?">
    No. Invitation delivery and response completion are separate. Check the recipient’s response status and the results page.
  </Accordion>
  <Accordion title="Does a missing sentiment score mean neutral feedback?">
    No. Analysis may not have run or may lack sufficient responses. Review the underlying answers and analysis state before interpreting the score.
  </Accordion>
  <Accordion title="Does Create Goal just copy text into the survey?">
    It starts a separate goal workflow that can create follow-up work. Review its company, content and result; generating survey analysis alone does not create the goal.
  </Accordion>
  <Accordion title="Why am I not getting survey response notification emails?">
    Response notification rules import disabled. Open the rule in Settings > Automation and enable it. Then check that the rule targets the right survey and team role, and that the fallback recipient is still an active team member or a valid address.
  </Accordion>
  <Accordion title="Are low-score alerts the same as survey reminders?">
    No. Reminders target nonrespondents, while low-score alerts notify the configured internal recipients about qualifying responses. Their thresholds and audience settings are separate.
  </Accordion>
</AccordionGroup>

## Continue reading

- [User guides](/user-guides/getting-started/index)
